...
Add-PSSnapin Siepe.Tools.PowerShell.PubSubSnapIn
	...
	#$LogFile = "$dirLogFolder\UploadGenevaBBCAXStockSplit.$logTime.txt" 
param(
[String]$LogFile 
...
Write-Output "`r`n################ $(get-date -format "yyyy/MM/dd hh:mm:ss") :: Executing Procedure:: Creating DataSet"
	$dbCmd.CommandText = "EXEC DataFeeds.dbo.pRefDataSetIU	@RefDataSetID		= 0 ,	@RefDataSetDate	    = '" +$FullDayString +"' ,	@RefDataSetType		= 'Geneva Equities' ,	@RefDataSource		= 'Advent Geneva' ,	@Label				= 'GenevaStockSplitXMLCreation'" 
	$dbCmd.ExecuteScalar()

	######## Get RefDataSetID ###########
	$dbCmd.CommandText = "SELECT TOP 1 RefDataSetID FROM  DataFeeds.dbo.vRefDataSet WHERE RefDataSetDate = '" +$FullDayString +"' AND RefDataSetType = 'Geneva Equities' AND RefDataSource = 'Advent Geneva' AND Label = 'GenevaStockSplitXMLCreation' ORDER BY 1 DESC"
	$RefDataSetID = $dbCmd.ExecuteScalar()	
	
	
	# Send Geneva-loadable FXRates XML to Geneva
	$dbCmd.CommandText = "EXEC Geneva.pExportBBCAXStockSplit @RefDataSetDate = '$FullDayString'"
	
	[String] $StartTime = (Get-Date) -f "yyyy-MM-dd hh:mm:ss"
	$dbCmd.ExecuteScalar() | Out-File -FilePath "$GV_PRD_WFM_DIR\CAXStockSplit.$str_Date.xml"
...
$dbCmd1.CommandText = "SELECT 1 FROM dbo.vGenevaActivityRun WHERE Source = 'C:\INPUT\CAXStockSplit.$str_Date.xml'
...
IF($Data -eq $null)
{
$dbCmd.CommandText = 	"EXEC DataFeeds.dbo.pRefDataSetIU 	@RefDataSetID		= " +$RefDataSetID +" , 	@RefDataSetDate	    = '" +$FullDayString +"' , 	@RefDataSetType		= 'Geneva Equities' , 	@RefDataSource		= 'Advent Geneva' ,	@Label		= 'GenevaStockSplitXMLCreation',	@StatusCode         = 'F'"
	$dbCmd.ExecuteScalar()
}
ELSE
{
$dbCmd.CommandText = 	"EXEC DataFeeds.dbo.pRefDataSetIU 	@RefDataSetID		= " +$RefDataSetID +" , 	@RefDataSetDate	    = '" +$FullDayString +"' , 	@RefDataSetType		= 'Geneva Equities' , 	@RefDataSource		= 'Advent Geneva' ,	@Label		= 'GenevaStockSplitXMLCreation',	@StatusCode         = 'P'"
	$dbCmd.ExecuteScalar()
}
...
#Write-PubSub -Subject "Process.Geneva.Daily.Upload.BloombergPricesFour" -Title "Complete - next Upload BBCAX Stock Div" -Description "Complete - next Upload BBCAX Stock Div"
