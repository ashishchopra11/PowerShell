...
#   $LogFile = "$dirLogFolder\UploadGenevaFXRates.$logTime.txt" 
...
Write-Output "`r`n################ $(get-date -format "yyyy/MM/dd hh:mm:ss") :: Executing Procedure:: Creating DataSet"
	$dbCmd.CommandText = "EXEC DataFeeds.dbo.pRefDataSetIU	@RefDataSetID		= 0 ,	@RefDataSetDate	    = '" +$FullDayString +"' ,	@RefDataSetType		= 'Marks' ,	@RefDataSource		= 'Advent Geneva' ,	@Label				= 'GenevaFXRateXMLCreation'" 
	$dbCmd.ExecuteScalar()

	######## Get RefDataSetID ###########
	$dbCmd.CommandText = "SELECT TOP 1 RefDataSetID FROM  DataFeeds.dbo.vRefDataSet WHERE RefDataSetDate = '" +$FullDayString +"' AND RefDataSetType = 'Marks' AND RefDataSource = 'Advent Geneva' AND Label = 'GenevaFXRateXMLCreation' ORDER BY 1 DESC"
...
$dbCmd1.CommandText = "SELECT 1 FROM dbo.vGenevaActivityRun WHERE Source = 'C:\INPUT\FXRate.$str_Date.xml'
...
IF($Data -eq $null)
{
$dbCmd.CommandText = 	"EXEC DataFeeds.dbo.pRefDataSetIU 	@RefDataSetID		= " +$RefDataSetID +" , 	@RefDataSetDate	    = '" +$FullDayString +"' , 	@RefDataSetType		= 'Marks' , 	@RefDataSource		= 'Advent Geneva' ,	@Label		= 'GenevaFXRateXMLCreation',	@StatusCode         = 'F'"
	$dbCmd.ExecuteScalar()
}
ELSE
{
$dbCmd.CommandText = 	"EXEC DataFeeds.dbo.pRefDataSetIU 	@RefDataSetID		= " +$RefDataSetID +" , 	@RefDataSetDate	    = '" +$FullDayString +"' , 	@RefDataSetType		= 'Marks' , 	@RefDataSource		= 'Advent Geneva' ,	@Label		= 'GenevaFXRateXMLCreation',	@StatusCode         = 'P'"
	$dbCmd.ExecuteScalar()
}
...
Remove-Variable dbConn
	...
	#Write-PubSub -Subject "Process.Geneva.Daily.Upload.BloombergPricesThree" -Title "Complete - next Upload BBCAX Stock Splits" -Description "Complete - next Upload BBCAX Stock Splits"
