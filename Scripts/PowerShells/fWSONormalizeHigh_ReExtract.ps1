...
## -- Normalize High Priority
	...
	Write-Output "`r`n################ $(get-date -format "yyyy/MM/dd hh:mm:ss:fff") :: Calling $dirSSISNormalizeWSO\NormalizeWSOPositions.dtsx `r`n Variable passed here are : `r`n  DataSetDate = $FullDayString `r`n PowerShellLocation = $ScriptName " | Out-File $LogFile -Append
