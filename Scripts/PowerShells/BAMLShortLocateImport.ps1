...
#****** Initialize variables ******
	$GenericImportJobID = 94
	$GenericNormaliztaionJobID = 51
	...
	$dirDataFeedsFolder = "$dirServicesDeliveryStoreFolder\BAML Short Locate"
	
	foreach ($strFileName in Get-ChildItem	 -Path $dirDataFeedsFolder | Where-Object {$_.Name -ilike "*Highland_loc_res*"})
	{
		$FileName = "$dirDataFeedsFolder\$strFileName"
		$FileName = $FileName.SubString(0,87)
